...
import com.ikksa.user.adapter.VariationAdapter
...
class ActProductDetails : BaseActivity() {
    ...
    private var viewAllDataAdapter: BaseAdaptor<RelatedProductsItem, RowFeatureproductgridBinding>? =
        null
    ...
    private fun loadVendorsData(vendors: Vendors) {
        ...
        Glide.with(this@ActProductDetails)
            .load(filterHttps(vendors.imageUrl)).into(productDetailsBinding.ivvendors)
    ...
    private fun loadRelatedProducts(relatedProducts: ArrayList<RelatedProductsItem>) {
        ...
        viewAllDataAdapter =
            object : BaseAdaptor<RelatedProductsItem, RowFeatureproductgridBinding>(
                this@ActProductDetails,
                relatedProducts
            ) {
                ...
                override fun onBindData(
                    ...
                    if(relatedProducts[position].productPrice!!.toDouble().equals(relatedProducts[position].discountedPrice!!.toDouble())){
    ...
    private fun loadProductDetails(productDetailsList: ProductDetailsData) {
        ...
        if(productDetailsList.productPrice!!.toDouble().equals(productDetailsList.discountedPrice!!.toDouble())){
        ...
        for (i in 0 until productDetailsList.productimages?.size!!) {
            val slideModel = SlideModel(filterHttps(productDetailsList.productimages[i].imageUrl))
            imageList.add(slideModel)
        }

        productDetailsBinding.imageSlider.setImageList(imageList, ScaleTypes.CENTER_CROP)
    ...
    class StartScreenAdapter(
        ...
        override fun instantiateItem(collection: ViewGroup, position: Int): Any {
            ...
            Glide.with(mContext).load(filterHttps(mImagelist[position].imageUrl)).into(iv)
    ...
    private fun setupProductVariationAdapter(variationList: ArrayList<VariationsItem>) {
        ...
        variationAdaper = VariationAdapter(
            ...
            if (s == "ItemClick") {
                ...
                var discountPriceVisibilityFlag = false;
                ...
                for (item in variationList) {
                    ...
                    if (productDetailsList?.isVariation == 1) {
                        if (variationList[i].qty == "0") {
                            ...
                            if(!discountPriceVisibilityFlag){

                                if(variationList[i].price!!.toDouble().equals(variationList[i].discountedVariationPrice!!.toDouble())){
                                    productDetailsBinding.tvProductDisprice.visibility = View.GONE
                                }else{
                                    productDetailsBinding.tvProductDisprice.visibility = View.VISIBLE
                                }

                                discountPriceVisibilityFlag = true;
                            }
